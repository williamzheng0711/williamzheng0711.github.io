...
- The renderer draws at most one translated copy of each feature in the current viewport, so a place cannot be duplicated at world-wrap edges; dateline-split countries such as Russia shift their polygon parts together. During horizontal dragging it keeps the current layer bundle visible and swaps in a complete background-rendered bundle after `moveend`, so countries do not refresh one piece at a time. Vertical dragging is clamped to the Web Mercator poles.
